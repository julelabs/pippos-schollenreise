...
export default function JourneyScene({destination,coins,avatar,active,onArrive}:{destination:number;coins:number;avatar:AvatarId;active:boolean;onArrive:()=>void}){
 ...
 const avatarData=AVATARS.find(item=>item.id===avatar)||AVATARS[0],avatarName=avatarData.name;
 ...
 return <section className={`landscape route-scene avatar-${avatar} ${landed?'landed-effect':''}`} aria-label={`${avatarName} in der ${avatarData.worldName}: ${destination} von 5 Sprüngen`}>
 <div className="scene-window" ref={viewport}><div className="scene-stage" style={{transform:`translateY(-${offset}px)`}}>
 <img className="world-art" src={import.meta.env.BASE_URL+avatarData.world} alt={`${avatarData.worldName}: Der Weg führt ${avatarData.destinationPhrase}.`} width={1086} height={1448}/>
 ...
 </div></div><div className="journey-label">{destination===5&&!moving?avatarData.arrival:`FÜNF SPRÜNGE ${avatarData.destinationPhrase.toUpperCase()}`}<span>{moving?'Huuuupf!':destination===5?'Direkt am Ziel.':active?`Nächstes Ziel: ${destination+1}`:'Folge den nummerierten Wegmarken.'}</span></div>{active&&<span className="fish-total" aria-label={`${coins} Münzen`}>🪙 {coins}</span>}<div className="scene-caption">{moving?'Von einer Etappe zur nächsten …':destination===5?`Geschafft! ${avatarName} ist am Ziel.`:active?`${destination} von 5 Sprüngen geschafft`:`Jede richtige Antwort bringt ${avatarName} ein Ziel weiter.`}</div>
 </section>
}
